...
from ciphers import (craft, craftrtk, craftrtk1, craftrtk2, craftrtk3, craftlinear, ein_even, em_even, em_even_4r, 
                     eout_even, ein_odd, em_odd, eout_odd, ein_even_6r)
...
def startsearch(tool_parameters):
    ...
    cipher_suite = {"craft" : craft.CraftCipher(),
                    ...
                    "em_even_4r" : em_even_4r.EmEven4r(),
    ...
    if tool_parameters["mode"] == 0:
        ...
        fileobj.close()        
        fileobj = open("result_em_even_4r.txt", "w")
        distribution = [[0 for k1 in range(len(possible_inputs))] for k2 in range(len(possible_inputs))]
        ...
        ntrails = [[0 for k1 in range(len(possible_inputs))] for k2 in range(len(possible_inputs))]
        ...
        for i in range(len_of_possible_ios):
            for j in range(len_of_possible_ios):
                ...
                tool_parameters["fixedVariables"]["z4"] = possible_inputs[j][0:-1]
                output_disp = "Output[%d]: %s" % (j, possible_inputs[j][0:-1])
                inout_disp = "%s \t %s" % (input_disp, output_disp)
                print(inout_disp)
                fileobj.write(inout_disp)
                fileobj.write("\n")
                pr, ntrails[i][j] = search1.computeProbabilityOfDifferentials(cipher, tool_parameters)
                distribution[i][j] = pr
                if pr > 0.0:
    ...
    fileobj.write("\nNumber of possible trails : %d out of %d" % (number_of_possible_trails, 10000))
    fileobj.close()
    return
