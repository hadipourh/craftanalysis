...
def startsearch(tool_parameters):
    ...
    if tool_parameters["mode"] == 0:
        ...
        fileobj.close()
        fileobj = open("result_ein_even_4r.txt", "w")
        ...
        for i in range(len(possible_inputs)):
            ...
            tool_parameters["fixedVariables"]["z4"] = possible_inputs[i][0:-1]
            ...
            fileobj.write("\n\n")
    fileobj.write("Distribution : %s" % distribution)
    for w in range(stweight, endweight):
        ...
        fileobj.write("]\n")    
    fileobj.write("\nNumber of possible trails : %d out of %d" % (number_of_possible_trails, 100))    
    fileobj.close()    
    return
