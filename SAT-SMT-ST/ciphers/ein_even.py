...
class EinEven(AbstractCipher):
    ...
    def createSTP(self, stp_filename, parameters):
        ...
        with open(stp_filename, 'w') as stp_file:
            ...
            x = ["x%d" % i for i in range(5)]
            y = ["y%d" % i for i in range(5)]
            z = ["z%d" % i for i in range(5)]
            # w = weight
            w = ["w%d" % i for i in range(4)]
            ...
            self.setupMixColumn(stp_file, x[4], y[4], wordsize)
            self.setupPermuteNibble(stp_file, y[4], z[4], wordsize)
