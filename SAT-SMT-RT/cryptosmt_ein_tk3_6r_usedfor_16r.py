...
from ciphers import (craft, craftrtk, craftrtk3, craftlinear, ein_tk2_7r)
...
def startsearch(tool_parameters):
    ...
    if tool_parameters["mode"] == 0:
        ...
        fileobj = open("tk3_possible_outputs.txt", 'r')
        possible_inputs = fileobj.readlines()
        fileobj.close()
        fileobj = open("result_ein_tk3_6r_outof_16r.txt", "w")
        ...
        for i in range(len(possible_inputs)):
            ...
            tool_parameters["fixedVariables"]["x6"] = possible_inputs[i][0:-1]
