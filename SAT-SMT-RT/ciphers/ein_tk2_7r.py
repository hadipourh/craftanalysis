...
class CraftCipherRTK2(AbstractCipher):
    ...
    def createSTP(self, stp_filename, parameters):
        ...
        with open(stp_filename, 'w') as stp_file:
            ...
            #for i in [2, 5]:
            #    self.setupMembership(stp_file, "s6", i)
            
            for i in range(rounds - 1):
                self.setupMixColumn(stp_file, x[i], y[i], wordsize)
                self.add_tweak(stp_file, y[i], tk[(i + 2)%4], z[i])
                self.setupPermuteNibble(stp_file, z[i], s[i], wordsize)
                self.setupSbox(stp_file, s[i], x[i + 1], w[i], wordsize)
            self.setupMixColumn(stp_file, x[rounds - 1], y[rounds - 1], wordsize)
            self.add_tweak(stp_file, y[rounds - 1], tk[((rounds - 1) + 2)%4], z[rounds - 1])
            self.setupPermuteNibble(stp_file, z[rounds - 1], s[rounds - 1], wordsize)
            self.setupSbox(stp_file, s[rounds - 1], x[rounds], w[rounds - 1], wordsize)
    ...
    def add_tweak(self, stp_file, y, tk, z):
        command = "ASSERT(" + z + " = BVXOR(" + tk + ", " + y + "));\n"
        stp_file.write(command)

    def setupPermuteNibble(self, stp_file, z, s, wordsize):
        command = ""
         # PermuteNibbles Layer
        # zr = PermuteNibbles(xr)
        # zr[i] = xr[PN[i]]
        for i in range(16):
            command += "ASSERT(" + s + "[%d:%d]" % (4*i + 3, 4*i) + \
                " = " + z + "[%d:%d]" % (4*self.PN[i] + 3,
                                         4*self.PN[i]) + ");\n"
        stp_file.write(command)

    def setupSbox(self, stp_file, s, x_out, w, wordsize):
        command = ""
        # Sbox layer
        for i in range(16):
            variables = ["{0}[{1}:{1}]".format(s, 4*i + 3),
