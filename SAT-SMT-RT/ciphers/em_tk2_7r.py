...
class CraftCipherRTK2(AbstractCipher):
    ...
    def createSTP(self, stp_filename, parameters):
        ...
        with open(stp_filename, 'w') as stp_file:
            ...
            s = ["s%d" % i for i in range(rounds + 1)]
            # w = weight
            w = ["w%d" % i for i in range(rounds + 1)]
            t = ["t0"]
            k0 = ["k0"]
            k1 = ["k1"]
            k2 = ["k2"]
            ...
            for i in [2, 5]:
                self.setupMembership(stp_file, "s0", i)
            
            for i in range(rounds):
                self.setupSbox(stp_file, s[i], x[i], w[i], wordsize)
                self.setupMixColumn(stp_file, x[i], y[i], wordsize)
                self.add_tweak(stp_file, y[i], tk[(i + 1)%4], z[i])
                self.setupPermuteNibble(stp_file, z[i], s[i + 1], wordsize)
            self.setupSbox(stp_file, s[rounds], x[rounds], w[rounds], wordsize)
